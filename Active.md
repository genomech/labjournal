...
* [Посмотреть](file:///dev/datasets/FairWind/_results/November/November_BGG_bam), какие буквы идут следом за левым краем рида - GATC или какие-то другие (!важно учесть ориентацию выравниваия!).
	...
	И ещё, можешь прогнать ещё раз свой скрипт, добавим паттер Tbridge и egdirbA (чтобы понять, много ли бриджей соединяется с геномом через А-тэйлинг?).
	Все это можно сделать только для одного или парочки образцов DNAse (например, 8 и 3), для всех не обязательно, поскольку в целом они выглядят (за исключением Пашиного) похоже.
