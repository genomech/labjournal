...
vcf1_header = ["VCF_GT", "VCF_AD", "VCF_DP", "VCF_GQ", "VCF_PL"]
...
hgmd_data = pd.read_csv(current_dir + "/annofit_data/new_hgmd.csv", sep='\t')
omim_data = pd.read_csv(current_dir + "/annofit_data/omim_pli.csv", sep='\t')
data = pd.read_csv(input_filename, sep='\t', converters={"Otherinfo13": str})
...
gotcha = gotcha.apply(lambda x: pd.Series(x, index=vcf1_header) if len(x) == len(vcf1_header) else pd.Series([float('NaN')] * len(vcf1_header), index=vcf1_header))
gotcha["VCF_GT"] = gotcha["VCF_GT"].apply(lambda x: "-" if type(x) is float else ("-" if (x == "0/0") else ("HOMO" if (len(set(x.split('/'))) == 1) else x)))
