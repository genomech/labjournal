...
def thread0(filename, output_dir):
	...
	seqs = pd.DataFrame(np.array([['-bridge-', 'CGGTGGC'], ['-egdirb-', 'GCCACCG']]), columns=['name', 'seq'])
	genome = '-genome-'
	...
	with Blister.Read(filename[1], 'rt', filename[0]) as input_file:
		with Blister.Timestamp(f"PARSING", filename[0]) as start_time:
			for record in SeqIO.parse(input_file, "fastq"):
				...
				line = line.replace("A-bridge-", "-a-bridge-")
				line = line.replace("-bridge-A", "-bridge-a-")
				line = line.replace("A-egdirb-", "-a-egdirb-")
				line = line.replace("-egdirb-A", "-egdirb-a-")
...
input_filenames = Blister.Input(["/dev/datasets/ngs_data/Chinese/*.fastq"])
if not input_filenames: exit()
output_dir = Blister.Dir("/dev/datasets/FairWind/_results/Chinese[1M]_analysis", create=True)
