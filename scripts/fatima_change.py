...
genes_diff = genes_diff[(genes_diff['q_value'] < 0.05) & ((genes_diff['value_1'] > 1000) | (genes_diff['value_2'] > 1000))]

#genes_diff["log2_fold_change"] = np.absolute(genes_diff["log2_fold_change"])
genes_diff.sort_values(by=['log2_fold_change'], axis=0, ascending=False, inplace=True)
...
#genes_diff.to_csv("/dev/datasets/FairWind/_results/Fatima/genes_plus_diff.csv", sep="\t", index=False)
print(genes_diff)
